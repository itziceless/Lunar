		...
		Function = function(callback)
			...
			if callback then
				...
				if success and clone and oldroot and proper then
					...
					local ray = workspace:Blockcast(CFrame.new(oldroot.Position.X, clone.CFrame.p.Y, oldroot.Position.Z), Vector3.new(3, entitylib.character.HipHeight, 3), Vector3.new(0, -10000, 0), rayCheck)
...
OwlCheck = AutoVoidDrop:CreateToggle({
		Name = 'Whisper check',
		Default = true,
		Tooltip = 'Doesnt drop items in void while whispered'
	})
end)
...
run(function()
	...
	AutoSuffocate = vape.Categories.World:CreateModule({
		...
		Tooltip = 'Places blocks on players'
	})
